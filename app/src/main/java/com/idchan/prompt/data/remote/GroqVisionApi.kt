...
class GroqVisionApi(private val context: Context) {
...
"""
    ...
    suspend fun analyzeImageWithGroq(
        imageUri: String,
        mode: PromptMode,
        ...
        try {
            ...
            val payload = mapOf(
                ...
                "messages" to listOf(
                    mapOf(
                        "role" to "user",
                        "content" to listOf(
                            mapOf("type" to "text", "text" to userInstruction),
            ...
            if (!response.isSuccessful || responseString.isEmpty()) {
                return@withContext Result.failure(Exception("Groq API Error: ${response.code} ${response.message}"))
            }

            val jsonMap = try {
                val root = gson.fromJson(responseString, Map::class.java) as Map<*, *>
                val choices = root["choices"] as? List<*>
                val firstChoice = choices?.firstOrNull() as? Map<*, *>
                val message = firstChoice?.get("message") as? Map<*, *>
                val content = message?.get("content") as? String ?: ""
                gson.fromJson(cleanJsonResponse(content), Map::class.java) as Map<String, Any>
            } catch (e: Exception) {
                null
            }

            val masterPrompt = jsonMap?.get("masterPrompt") as? String
                ?: "A detailed image-generation prompt reverse engineered by Groq AI vision engine"
            val negativePrompt = jsonMap?.get("negativePrompt") as? String
                ?: "distorted anatomy, extra fingers, blurry details, incorrect proportions, low resolution, unnatural skin"
